...
### Use this python when launching the code: /opt/anaconda3/envs/dc_p27/bin/python
    ...
    for instance in range(1,instances):
        for state in states:
            ...
            if 'random' in state:
                ...
                #checkpoint = torch.load(modelpth+'checkpoint_dc%d_epoch0.pth.tar' % instance)['state_dict']   #### When loading the random state the top_layer is not set and cannot be matched to the alexnet model structure as loaded from the model folder
            else:
                checkpoint = torch.load(modelpth+'checkpoint_dc%d_epoch100.pth.tar' % instance)['state_dict']
            ...
            model.cuda()
            image_pth = '/data/algonautsChallenge2019/Training_Data/92_Image_Set/92images' 
            act = get_activations(image_pth)

            with open('/data/multiple_deepcluster/activations_niko92_imgs/activations_%s_dc%d.pickle' % (state,instance), 'wb') as handle:
